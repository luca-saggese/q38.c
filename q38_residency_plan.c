...
enum {
    Q38_RESIDENCY_OWNERSHIP_RESIDENT = 1,
};
...
static int compare_entries(const void *left, const void *right) {
    const q38_residency_plan_entry *a = left;
    const q38_residency_plan_entry *b = right;
...
static bool span_has_ple_between(
    ...
    for (uint64_t i = 0; i < model->n_tensors; ++i) {
        const q38_tensor *candidate = &model->tensors[i];
...
bool q38_residency_plan_build(
    ...
    for (size_t i = 0; i < plan->entry_count; ++i) {
        const q38_residency_plan_entry *entry = &plan->entries[i];
        if (!plan->span_count) {
            plan->spans[0] = (q38_residency_plan_span){
                entry->file_offset, entry->bytes, i, 1};
            plan->span_count = 1;
            continue;
        }
        q38_residency_plan_span *span = &plan->spans[plan->span_count - 1];
